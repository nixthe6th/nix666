...
```bash
nix when [in|until|since|add|deadline|list] [args]
```
...
| Command | Description |
|---------|-------------|
| `in <duration>` | Show time in the future |
| `until <date>` | Days until a deadline |
| `since <date>` | Days since a date |
| `add <date> <duration>` | Add time to a date |
| `deadline <name> <date>` | Save a deadline |
| `list` | Show all saved deadlines |
...
- `1w` — 1 week
- `3mo` — 3 months
- Combined: `2h30m`, `1d12h`

### Examples

```bash
nix when in 2h30m              # What time will it be?
nix when until 2026-02-15      # Days until Feb 15
nix when until 02-15           # Short date format
nix when since 2026-01-01      # Time elapsed
nix when add today 7d          # Add 7 days to today
nix when deadline "Launch" 02-15   # Track a deadline
