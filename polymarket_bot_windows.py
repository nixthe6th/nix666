...
class PolymarketArbitrageBot:
    ...
    def __init__(self):
        ...
        if not Config.PRIVATE_KEY and not Config.DRY_RUN:
            logger.error("ERR PRIVATE_KEY not set! Add to .env file or enable DRY_RUN")
            sys.exit(1)
    ...
    def fetch_markets(self):
        ...
        try:
            ...
            logger.debug(f"[STAT] Fetched {len(markets)} markets")
            return markets
    ...
    def execute_trade(self, opportunity, sizes):
        ...
        # Log trade details
        logger.info(f"[HIT] ARBITRAGE OPPORTUNITY FOUND!")
        ...
        try:
            ...
            logger.info(f"   OK YES order placed: {yes_order}")
            ...
            logger.info(f"   OK NO order placed: {no_order}")
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                logger.info(f"[FIND] Checking {len(target_markets)} BTC markets...")
